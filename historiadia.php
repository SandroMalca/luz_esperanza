...
#myTable #datos{
  ...
  <body>
    <header class="clearfix">
      <div align="center">
      <b style="font-size: 20px;font-family: Arial, Helvetica, sans-serif;">Historias por día<b>
      <div>
    </header>
    <main>
    <br><br>
      <table id="myTable">
        <tr class="header">
          ...
          $resultadoHistoriaDia = $conexion -> query("select count(historia.id) as historia, historia.fecha as fecha from historia group by historia.fecha order by historia.fecha desc");
        while ($mostrarHistoriaDia=mysqli_fetch_array($resultadoHistoriaDia))  {

          $html.='<tr>
          <td>'
  			  .date('d-m-Y', strtotime($mostrarHistoriaDia['fecha'])).
          '</td>
          <td>'
          .$mostrarHistoriaDia['historia'].
          '</td>
        </tr>';
    }
