    ...
    <br><br>
      <table id="myTable">
        <tr class="header">
          ...
          $resultadoPacienteMes = $conexion -> query("select count(persona.id) as pacientes, monthname(persona.fecha) as mes, year(persona.fecha) as ano from persona where persona.id_tipopersona=2 group by mes, ano order by ano desc, mes desc ");
        while ($mostrarPacienteMes=mysqli_fetch_array($resultadoPacienteMes))  {

          $html.='<tr>
          <td>'
  			  .$mostrarPacienteMes['mes'].
          '</td>
          <td>'
          .$mostrarPacienteMes['ano'].
          '</td>
          <td>'
          .$mostrarPacienteMes['pacientes'].
          '</td>
        </tr>';
    }
